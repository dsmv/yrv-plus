...
//     end
  ...
  always @ (posedge clk) begin
    if (mem_trans[0]) begin
      mem_rdata[31:0] <= mcu_mem[mem_addr[13:2]];
      end
    end

  always @ (posedge clk) begin
    if (mem_wr_byte[3]) mcu_mem[mem_addr_reg[13:2]][31:24] <= mem_wdata[31:24];
    if (mem_wr_byte[2]) mcu_mem[mem_addr_reg[13:2]][23:16] <= mem_wdata[23:16];
    if (mem_wr_byte[1]) mcu_mem[mem_addr_reg[13:2]][15:8] <= mem_wdata[15:8];
    if (mem_wr_byte[0]) mcu_mem[mem_addr_reg[13:2]][7:0] <= mem_wdata[7:0];
    end
...
//     initial $readmemh("code_demo.mem32", mcu_mem);
// `endif
